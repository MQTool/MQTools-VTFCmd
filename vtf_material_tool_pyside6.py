...
class L4D2ProcessingThread(QThread):
    ...
    def run(self):
        try:
            ...
            # 检查基础色贴图是否有真实透明度
            has_transparency = self.has_real_transparency(base_texture_img)
            if has_transparency:
                self.progress.emit("检测到真实透明度，将在VMT中添加$translucent参数")
            
            # 检测materials目录并生成相应的VMT内容
            vmt_content = self.generate_l4d2_vmt_with_materials_detection(has_transparency=has_transparency)
...
\t$phongfresnelranges\t\t"[0.1 0.8 1.0]"
    ...
    def generate_l4d2_vmt_with_materials_detection(self, has_transparency: bool = False) -> str:
        """生成VMT内容 - 带materials目录检测，按照PBR-2-Source原版逻辑
        
        Args:
            has_transparency: 是否有真实透明度，如果为True则添加$translucent参数
        """
        ...
        # 构建透明度参数
        transparency_param = "\t$translucent\t\t\t1\n" if has_transparency else ""
